...
-- Handle existing medium font size values by converting them to default
UPDATE posts SET font_size = 'default' WHERE font_size = 'medium';

-- Update font_size enum to remove medium
DO $$
BEGIN
  -- Create a new enum without medium
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'font_size_new') THEN
    CREATE TYPE font_size_new AS ENUM ('default', 'large');
    
    -- Update the posts table to use the new enum
    ALTER TABLE posts ALTER COLUMN font_size TYPE font_size_new USING font_size::text::font_size_new;
    
    -- Drop the old enum and rename the new one
    DROP TYPE font_size;
    ALTER TYPE font_size_new RENAME TO font_size;
  END IF;
END$$;
